...
import { ShieldCheck, Truck, CreditCard, ArrowRight, Zap } from 'lucide-react';
...
export default function CheckoutPage() {
  ...
  const handlePlaceOrder = async () => {
    if (!address || !city || !zip) {
      alert('Please provide your delivery coordinates (Address).');
      return;
    }
    ...
    try {
      ...
      const order = await fetchApi('/order/create', { 
        method: 'POST',
        body: JSON.stringify({ shippingAddress: `${address}, ${city}, ${zip}` })
      });
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: '3rem' }}>
        ...
        <div className="glass-panel" style={{ padding: '2.5rem' }}>
          <h3 style={{ fontSize: '1.5rem', marginBottom: '2rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            <Truck size={20} color="var(--primary-color)" /> 01. DELIVERY COORDINATES
          </h3>
          
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
             <div>
               <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.8rem', color: 'var(--text-secondary)' }}>STREET ADDRESS</label>
               <input type="text" className="input-field" value={address} onChange={e => setAddress(e.target.value)} placeholder="Neo-Tokyo, Sector 7..." />
             </div>
             <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
               <div>
                 <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.8rem', color: 'var(--text-secondary)' }}>CITY / HABITAT</label>
                 <input type="text" className="input-field" value={city} onChange={e => setCity(e.target.value)} placeholder="Cyber City" />
               </div>
               <div>
                 <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.8rem', color: 'var(--text-secondary)' }}>ZIP / SECTOR CODE</label>
                 <input type="text" className="input-field" value={zip} onChange={e => setZip(e.target.value)} placeholder="10001" />
               </div>
             </div>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <div className="glass-panel" style={{ padding: '2rem' }}>
            ...
            <div style={{ margin: '1.5rem 0', borderTop: '1px solid var(--border-color)', paddingTop: '1.5rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            ...
            <button 
              ...
              style={{ width: '100%', padding: '1.5rem', fontSize: '1.25rem', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '1rem' }}
            >
              {processing ? 'EXECUTING...' : 'FINALIZE TRANSACTION'} <ArrowRight size={20} />
            </button>
            <p style={{ textAlign: 'center', marginTop: '1rem', fontSize: '0.7rem', color: 'var(--text-secondary)' }}>
              By clicking finalize, you agree to the Neural-Transfer Terms of Service.
            </p>
          </div>

          <div className="glass-panel" style={{ padding: '1.5rem', textAlign: 'center' }}>
            <Link href="/cart" style={{ color: 'var(--primary-color)', fontSize: '0.9rem', fontWeight: 700 }}>
