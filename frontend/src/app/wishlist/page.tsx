...
import { Heart, Trash2, ShoppingCart, ArrowLeft, Lock } from 'lucide-react';
  ...
  useEffect(() => {
    const checkAndLoad = async () => {
      ...
      try {
        ...
        setTimeout(() => {
          window.dispatchEvent(new CustomEvent('wishlist-update', { detail: data.length }));
        }, 0);
      } catch (err: any) {
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '60vh', flexDirection: 'column', gap: '1rem' }}>
        ...
        <p style={{ color: 'var(--text-secondary)' }}>Decrypting Secure Wishlist...</p>
    ...
    return (
      <div className="container" style={{ padding: '8rem 0', textAlign: 'center' }}>
        <div className="glass-panel" style={{ padding: '4rem', maxWidth: '600px', margin: '0 auto' }}>
          <Lock size={60} color="var(--error-color)" style={{ marginBottom: '2rem' }} />
          <h1 style={{ fontSize: '2.5rem', fontWeight: 900, marginBottom: '1.5rem' }}>Access Denied</h1>
          <p style={{ color: 'var(--text-secondary)', marginBottom: '3rem', fontSize: '1.1rem' }}>
            Wishlist protocols are encrypted and locked to individual user identities. Please authenticate to access your saved modules.
          </p>
          <Link href="/login" className="btn-primary" style={{ padding: '1rem 3rem' }}>Authenticate Terminal</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="container" style={{ padding: '5rem 0' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4rem' }}>
        <div>
          <h1 style={{ fontSize: '3rem', fontWeight: 900, marginBottom: '0.5rem' }}>Personal Wishlist</h1>
          <p style={{ color: 'var(--text-secondary)' }}>Securely stored hardware selections for your identity.</p>
        </div>
      ...
      {wishlistItems.length === 0 ? (
        <div className="glass-panel" style={{ padding: '6rem', textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: '2rem', marginBottom: '1rem' }}>No Data Found</h2>
          <p style={{ color: 'var(--text-secondary)', marginBottom: '3rem' }}>Your personal wishlist buffer is currently empty.</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '2.5rem' }}>
          {wishlistItems.map((item) => {
            ...
            if (!prod) return null;
            return (
              <div key={item.id} className="product-card" style={{ padding: '1.5rem' }}>
                <div className="product-image-wrapper" style={{ height: '220px', marginBottom: '1.5rem', background: 'rgba(255,255,255,0.02)', borderRadius: '8px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                  <div>
                    <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', fontWeight: 800, textTransform: 'uppercase' }}>SAVED MODULE</p>
